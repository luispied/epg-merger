...
Muchos nombres de canal (ej. "E!", "TBS") existen varias veces en el EPG mergeado — un feed distinto por país, o variantes regionales de EE.UU. (East/West/Pacific). El script elige uno automáticamente para `playlist.m3u8` (una sola entrada por canal, sin duplicados), pero **también incluye hasta 4 alternativas en `my_epg.xml.gz`**, cada una con su `display-name` anotado entre paréntesis — país + feed regional si se detectaron (ej. `"TBS East"` vs `"TBS Pacific"`), o si no, el propio `channel_id` (y si dos alternativas quedarían con la misma etiqueta, ej. dos ".us" sin feed detectado, se les agrega un fragmento del `channel_id` para poder distinguirlas).
